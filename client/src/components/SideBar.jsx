...
const SideBar = ({}) => {
  const {getUsers, users, selectedUsers, setSelectedUsers, unseenMessages, setUnseenMessages} = useContext(ChatContext);
  const {logout, onlineUser} = useContext(AuthContext);
  const navigate = useNavigate(); // i will use this for edit profile

  const [input, setInput] = useState(false); // used to search
  const filteredUsers = input ? users.filter((user) => { return user.FullName.toLowerCase().includes(input.toLowerCase())})  : users;

  useEffect(() => {
    getUsers();
  }, [onlineUser]) // whenever online users change i will fetch my users again
    ...
    <div className={`bg-[#8185b2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white 
      ...
      <div className='flex flex-col'>
        {filteredUsers.map((user, index) =>(
            <div onClick={()=> {setSelectedUsers(user); setUnseenMessages(prev => ({...prev, [user._id]:0}))}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm 
                           ${selectedUsers?._id === user._id && 'bg-[#28142]/50'} `}>
                ...
                {(unseenMessages || {})[user._id] > 0 && <p className='absolute top-4 right-4 text-xs h-5 w-5 bg-violet-500/50 flex justify-center items-center rounded-full'>{unseenMessages[user._id]}</p>}
            </div>
        ))}
      </div>
    </div>
